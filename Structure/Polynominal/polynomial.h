...
private:
    ...
    int expo;  // 지수
public:
    TermOperate(int coeff = 0, int expo = 0); // 생성자
    ...
    // 연산자 오버로딩 -> 다항식 덧셈 뺄셈
    TermOperate operator+(const TermOperate& other) const; // 다항식 덧셈
    TermOperate operator-(const TermOperate& other) const; // 다항식 뺄셈
    // TermOperate : 반환되는 타입
